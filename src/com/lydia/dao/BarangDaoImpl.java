...
public class BarangDaoImpl implements DaoService<Barang> {
    ...
    public ObservableList<Barang> showTopData(String object, String object2) {
        ...
        try {
            try (Connection connection = Koneksi.createConnection()) {
                String query
                        = "SELECT b.kd_Barang,b.nm_Barang,SUM(dt.jml) AS 'Jumlah Terjual' ,((SUM(dt.jml) )*dt.saling_price) AS 'Total' from transaksi t JOIN detail_transaksi dt ON t.kd_Transaksi = dt.transaksi_kd_Transaksi JOIN Barang b ON b.kd_Barang = dt.barang_kd_Barang where t.tgl_Transaksi BETWEEN  t.tgl_Transasi = ? AND t.tgl_Transasi = ? GROUP BY b.kd_Barang, b.nm_Barang ORDER BY SUM(dt.jml) DESC";
                PreparedStatement ps = connection.prepareStatement(query);
